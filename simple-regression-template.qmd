...
`illiteracy` (y-axis), displaying the `name` of each state (label).
...
__True or False__: based on the states data and the preceding scatter plot ...
...
Based on the output of your model `mod1`, provide a verbal interpretation of 
...
of the linear model---using the "slope" and "intercept" terms. 
...
larger than or equal to 55 percent. This mutation requires a logical 
condition `hs_grad >= 55`

```{r}
# mutate: add variable "hs_plus55"
states <- states |> 
  mutate(_____________________)
```
...
Interpretation of `murder` coefficient:

Interpretation of `hs_plus55` coefficient:



__Challenge__: Obtain a scatter plot to visualize the parallel lines associated
to the regression model fitted above.
